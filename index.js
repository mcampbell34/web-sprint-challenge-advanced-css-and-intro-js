...
function addFeedback (artists, index, name){
  artists[index].name = name;
  return reviews;
}
console.log(addFeedback(artists, 9, `Vincent Van Dough`)); 
...
function get20s(data) {
  names = []
  data.forEach(artist => {
      const getYears = artist.years.split(" ")
      if (parseInt(getYears[0]) >= 1900 && parseInt(getYears[2]) <= 2000)
...
function addArtist(id, name, years, genre, nationality, bio){
  ...
  console.log(addArtist(20, 'Marcus', 1996-2020, 'Web Design', 'African American', 'Im currently a web student who is looking to learn more about code. My faviorte colors are blue and gold. And I love tacos'))
